...
md"""
# Step 1: Ingest data
Before we can explore data, we need to:
1. identify a data source, 
1. construct a *query*, a command that specifies what data we want to access,
1. download the results of that query, 
1. store the data locally, and
1. read the data into memory.  

For this lab, I've chosen the "Planetary Systems table" (abbreviated "ps") from 
the [NASA Exoplanet Archive](https://exoplanetarchive.ipac.caltech.edu/).  
They provide [documentation](https://exoplanetarchive.ipac.caltech.edu/docs/API_PS_columns.html) about the meaning of each entry.
We'll discuss querying databases later in the class.  
For now, I've included code to download the dataset.  
But I encourage you not to get distracted by the details of that this week.  
"""

# ╔═╡ dfd5870a-d3fd-43cf-afef-a46972093e30
md"""
The Exoplanet Archive (and many other astronomical databases) can be queried using the *Table Access Protocol (TAP)* which is maintained by the International Virtual Observatory Alliance (IVOA).  
If we just wanted to download every column and every row from the *table* named `ps`, we could request that using the url
`https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+*+from+ps&format=tsv`.  
...
The Exoplanet archive provides a [quick summary of how to build TAP query commands](https://exoplanetarchive.ipac.caltech.edu/docs/TAP/usingTAP.html#sync).
...
md"""
Since querying a database and downloading data over the internet is slow (relative to the computations we'll be performing), we only want to download the data once.  
Therefore, we'll check if the data file already exists and only query the database if the file is missing.

The download command, also requires that we specify a path and file name for where downloaded data is to be stored.
"""
...
That way if something fails, we'll know it right away, rather than having to debug more complicated code later.

Once we confirm that we have the results of the query stored on disk, now we can load it into RAM (active memory).  Julia has a fast and flexible `CSV` reader.  
We'll specify that the data is to be stored in a [`DataFrame`](https://juliadatascience.io/dataframes).  
...
Before we start trying to analyze the data, we need to make sure that the file wasn't corrupted, that there wasn't a problem reading in the data, and that there isn't low-quality data that would invalidate our analysis.  
...
md"""
The first column (labeled `variable`) in the table above is the column name and the later columns contain summary statistics for that column's data.  
By default, describe computes the mean, minimum, median, maximum, the number of missing entries, and the the type of variables in that column.  
"""

# ╔═╡ 601b235c-9979-4a63-b2bb-8fcf332ad773
md"""
...
md"""
**Q1a:**  How many columns of the `df_raw` table contain a value stored as some kind of a real number?  Enter your responce by replacing the `missing` variable below.
"""

# ╔═╡ 623d0ab8-5cc5-4c92-bb6f-d452397e6c96
response_1a = missing

# ╔═╡ 050f2c0f-4c71-4251-8c33-307d1446cb51
md"""
**Q1b:** Notice that `:default_flag` only takes on the value of `1`.  
That's because our original query specified that we should only download rows for which that is the case.  Identify a few more columns that use a number to store [categorical data](https://en.wikipedia.org/wiki/Categorical_variable), rather than an actual measurement.
"""

# ╔═╡ 72dd28be-abe5-4a79-9cbe-3210d28b2d3c
response_1b = [:default_flag, missing, missing, missing]

# ╔═╡ 02440c11-3343-4270-ad4c-e317449ceabc
md"""
**Q1c:** Identify a few columns that contain suspicious values.  Explain why you are suspicious of those values.
"""

# ╔═╡ 1f0ac640-0e2c-4570-a5ba-199c4c756153
response_1c = missing # Replace missing with a string containing one or two sentances.

# ╔═╡ 9c90339a-bef9-4d0e-8cf3-e735fd2e793b
md"""
...
md"""
**Q1d:**  How many columns of the `df_raw` table contain some kind of string?
"""

# ╔═╡ 699b904a-7477-49cf-a6d5-a1fbdd823df4
response_1d = missing

# ╔═╡ b40eef32-8488-4f67-9e96-dc37079fcc48
md"""
Some columns contain a simple string that is some type of label (e.g., a planet name or star identifier).  
Some labels are intended to be unique (e.g., a planet name), while others contain a [categorical variable](https://en.wikipedia.org/wiki/Categorical_variable) (e.g., discovery method).
"""

# ╔═╡ 7fb56993-6486-4907-9c30-5c7f07168b5e
md"""
**Q1e:** The column `:nunique` shows the number of unique values in a given column of data.  Identify a few columns that use a string to store categorical data.
"""

# ╔═╡ 76582e98-4ecf-4a44-904f-be3d4cd382a8
response_1e = [missing, missing, missing]

# ╔═╡ e9f657c3-745f-4c41-8147-57c363532092
md"""
Others columns contain HTML code for links to references (e.g., `pl_refname`).
In those cases, we might want to convert them into an HTML type, so they display more nicely.
"""

# ╔═╡ d41cfb05-4c36-4fd6-8b79-9fd8b4ea3d6d
md"""
Some columns contain numeric data with formatting information (e.g., right ascension in `rastr` or a number and errorbar in `pl_trandepstr`).  
In this case, those strings are merely intended to be helpful for displaying the values, as the actual numeric data is contained in other columns.  
So we could eliminate those columns.

Other columns contain dates.
If we wanted to analyze the date in these columns, then we'd want to parse those strings and convert the data into an appropriate type (e.g., julia's `Date` or something fancier from [AstroTime](https://github.com/JuliaAstro/AstroTime.jl)).
However, often a small fraction of the data has annoying properties. 
For example, if we were to try to convert the data in `pl_pubdate` into proper `Date` types, we'd get an error.
...
For now, we'll just skip that time-consuming step and focus on columns with numeric data or simple strings.  
...
md"""
**Q2a:** Consider the number of missing entries for each column.  What is the main reason for such dramatic differences between the orbital period, mass, radius, and density?
"""

# ╔═╡ 4d312b47-904c-4a34-9477-c7266a7b2494
response_2a = missing  # TODO: Replace missing with a string with your response.

# ╔═╡ 4fce988d-c822-448e-83a9-e60caafb7026
md"""
Now, we'll make histograms to help visualize the data in some of the columns.
...
md"""
**Q2c:** Before looking, what do you predict this histogram will look like?
What will be the range of values?
Where will the center bed?
Will the distribution be nearly Gaussian?
"""
...
tip(md"""
Often in labs, you'll be asked to *predict* what you expect will happen.  
In these cases, the thought counts more than getting it "right".
Usually, a later portion of the lab will include a calculation or plot that will provide additional information or can test your prediction. 
Then, you'll be asked to reassess your prediction.
At that point, you may realize your original prediction was wrong.  
That's ok!  
In that case, please explain your new understanding and how that compares to your original prediction. 
But please don't go back and change your original prediction.  
I want to understand better how you're thinking through these issues.
...
Try several values for `number_of_bins` (on line two of the above block) to see what happens as you increase the number of bins (e.g., 10, 20, 40, 80, 400).

**Q2c:** What is the risk of using too few bins?  What is the risk of using too many bins?  
"""

# ╔═╡ 6f698f9d-b2da-4127-9975-6d74a7ef9265
response_2d = missing

# ╔═╡ 5b9bc8eb-377a-4074-8feb-f4a5cfc65b0f
md"""
...
# ╔═╡ 55ad5939-9237-4285-822c-a159b0320476
md"""
**Q2f:** How does your current understanding of the distribution of $m \sin i$ compare to your original predictions for its distribution from Q2c?  
"""

# ╔═╡ 44c77065-6f84-4803-bb1c-fed4b887be3e
response_2f = missing

# ╔═╡ 1e4f2c95-c505-4c7b-a58d-a33c9e4187e5
md"""
# Step 4:  Identify potential relationships in data

Often, there are interesting relationships between the different columns of data.
In a statistics class, you would learn about statistics (e.g., correlation coefficient) that can be computed from the data and tests to evaluate their significance.  
Here we'll focus on performing a visual inspection of possible correlations.
...
md"""
**Q3a:** What's one advantage and one disadvantage of using a 2-d kernel density estimator rather than a simple scatter plot?  
"""

# ╔═╡ ecf2fd37-f911-4057-98d8-4c12321e2b52
response_3a = missing

# ╔═╡ d7b7d7b2-4320-4494-b523-59f1ce0c9a7f
md"""
**Q3b:**  Using the code for the above two examples as a starting point, make a plot that shows the relationship of the orbital period and orbital eccentricity (column name `pl_orbeccen`) of known exoplanets.  
Once you have a basic working plot, make sure that the axis labels match what you're plotting.
Then think about how you could improve its effectiveness at visualizing the information.  
For example, think about:
- Which axis (if any) should be on a log scale?
- Does a scatter plot or density estimate work better in this case?
- If you use a scatter plot, how large should each point be?
- If you use contours, how many levels should you include and what bandwidths give a useful density estimate?  
Update your plot so it more effectively communicates the relationship between orbital period and eccentricity.
"""
...
md"""
**Q3c:**  
What problem arises if you use a scatter plot above?
What problem arises if you use contours from a kernel density estimator for the plot above?
"""

# ╔═╡ 82f2de9f-0a3b-41cf-9d56-47b6e7070a3d
response_3c = missing

# ╔═╡ 9d11daed-54ec-41a7-8814-483189b9d93a
md"# Setup, Helper & UI Code"
